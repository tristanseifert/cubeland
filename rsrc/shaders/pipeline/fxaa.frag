...
in vec2 TexCoords;
in vec4 TexCoordsPosPos;

// output the actual colour
layout (location = 0) out vec3 FragColour;

// gamma value to correct with
uniform float gamma;

// some information the FXAA shader uses
uniform vec2 rcpFrame; // (1/w, 1/h)
uniform vec4 rcpFrameOpt; // (-N/w, -N/h, N/w, N/h)
uniform vec4 rcpFrameOpt2; // (-2/w, -2/h, 2/w, 2/h)

// FXAA configurables
uniform float fxaaSubpixelAliasing; // fxaaQualitySubpix (0.74)
uniform float fxaaEdgeThreshold; // fxaaQualityEdgeThreshold (0.166)
uniform float fxaaEdgeThresholdMin; // fxaaQualityEdgeThresholdMin (0.0833)
uniform float fxaaEdgeSharpness; // fxaaConsoleEdgeSharpness (8.0)

// we can disable antialiasing
uniform int doFXAA; // 0 = no FXAA, 1 = yes FXAA

// Final render output
uniform sampler2D inSceneColours;

// configurables for FXAA shader
#define FXAA_PC					1
...
//	if(doFXAA == 1) {
		...
		vec4 fsaaColour = FxaaPixelShader(TexCoords, TexCoordsPosPos,
										  ...
										  inSceneColours, rcpFrame,
										  rcpFrameOpt, rcpFrameOpt2,
										  rcpFrameOpt2,
										  fxaaSubpixelAliasing,
										  fxaaEdgeThreshold,
										  fxaaEdgeThresholdMin,
										  fxaaEdgeSharpness,

										  // console-specific quality and vectors
										  fxaaEdgeThreshold,
										  fxaaEdgeThresholdMin,
										  vec4(0, 0, 0, 0));

		// apply gamma correction
		vec3 original = texture(inSceneColours, TexCoords).rgb;
		// vec3 correctedColour = pow(abs(fsaaColour.rgb - original), vec3(1.0 / gamma));
