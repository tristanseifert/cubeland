 ...
 */
void Buffer::bufferData(const std::size_t size, const void *data) {
    this->bind();
 ...
 */
void Buffer::replaceData(const std::size_t offset, const std::size_t size, const void *data) {
    this->bind();
    glBufferSubData(bufferTypeGL(), (GLintptr) offset, (GLsizeiptr) size, data);
}

/**
 * Efficiently replaces the contents of the buffer.
 *
 * Following some guidance from NVidia documentation, we first call glBufferData() with a null
 * pointer to re-allocate the buffer, then glBufferSubData() into that. Hopefully the driver is
 * somewhat smart about this
 */
void Buffer::replaceData(const std::size_t size, const void *data) {
    this->bind();
    glBufferData(bufferTypeGL(), (GLsizeiptr) size, nullptr, usageTypeGL());
    glBufferSubData(bufferTypeGL(), 0, (GLsizeiptr) size, data);
}

/**
 * Reserves the specified amount of memory in the buffer.
 */
void Buffer::reserveData(const std::size_t size) {
    this->bind();
    glBufferData(bufferTypeGL(), (GLsizeiptr) size, NULL, usageTypeGL());
}

/**
 * Attempts to map the buffer into virtual memory, using the specified policy. This call may fail
 * for some reason, and if it does, NULL is returned.
 *
